...
يمكنك نقل اعضاء لكروبك 🛎
من اي كروب اخر عام  ☄

Creator : @Aaaapppoii *""",reply_markup=inline ,parse_mode="markdown")
...
def call(call):
    ...
    if call.data =="a1":
        msg=bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="*قوم بارسال رابط الكروب المراد النقل منه *🖲",parse_mode="markdown")
        bot.register_next_step_handler(msg, statement)
def statement(message):
    Fromgrob = message.text
    msg =bot.send_message(chat_id=message.chat.id,text="*قوم بارسال رابط الكروب المراد النقل له*🛎",parse_mode="markdown")
